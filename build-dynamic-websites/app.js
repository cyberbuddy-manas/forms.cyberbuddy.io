...
setupDynamicInput('timeOther', 'timeRadio', 'timeLabel');
...
function sendMessage() {
  ...
  const fields = [name1, number, mail];
  ...
  const time = getRadioValue('time');
  if (!time) {
    document.querySelector('input[name="time"]').focus();
    snackbar("Please select your time preference.");
    return;
  }
  ...
  db.ref("build-dynamic-website").push({
    name: name1.value,
    number: number.value,
    mail: mail.value,
    github: github.value,
    ...
    .then(() => {
      ...
      document.getElementById("username").textContent = name1.value;
      window.scrollTo(0, 0);
    })
    .catch(error => {
      console.error(error);
